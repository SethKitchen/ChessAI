...
namespace SethRocks
{
    ...
    internal static class Book
    {
        ...
        internal static Move probe(Position pos, string filename, bool pickBest)
        {
            ...
            using (Stream fs = typeof(Engine).GetTypeInfo().Assembly.GetManifestResourceStream("SethRocks.book.bin"))
            {
                if (fs == null)
                {
                    bookNotExists = true;
            ...
            try
            {
                System.Reflection.Assembly bookAssembly = System.Reflection.Assembly.Load("SethRocksBook");
                using (Stream fs = bookAssembly.GetManifestResourceStream("SethRocksBook.book.bin"))
                {
                    UInt64 size = (UInt64)(fs.Length / SIZE_OF_BOOKENTRY);
                    using (BinaryReader br = new BinaryReader(fs))
                    {
